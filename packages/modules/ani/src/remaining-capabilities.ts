...
export function speakText(text: string, state: VoiceTtsState): Promise<void> {
  return new Promise((resolve, reject) => {
    if (!state.supported || !window.speechSynthesis) {
      ...
      estimatedMinutes:
        difficulty === "beginner"
          ...
          : difficulty === "intermediate"
            ...
            : 40,
      prerequisites:
        difficulty === "beginner" ? [] : ["Basic programming concepts"],
      objectives: [
      ...
      steps: [
        {
          ...
          content:
            "System architecture is the fundamental organization of a system embodied in its components, their relationships to each other, and to the environment.",
          keyPoints: [
        ...
        },
        {
          ...
          title: "Key Patterns",
          content:
            "Common patterns include layered architecture, microservices, event-driven, and modular monoliths. Each has distinct tradeoffs.",
          keyPoints: [
            "Layered: simple but rigid",
            "Microservices: scalable but complex",
            "Event-driven: decoupled but hard to debug",
          ],
          completed: false,
        },
        {
          ...
          content:
            "Every architectural decision involves tradeoffs between scalability, maintainability, complexity, and cost.",
          keyPoints: [
            "No perfect solution",
            "Context matters",
          ...
          checkQuestion:
            "Which pattern best suits a small team building an MVP?",
          checkAnswer:
            "Modular monolith — simpler ops, easier refactoring later.",
          completed: false,
        },
      ],
    },
      ...
      steps: [
        {
          ...
          content: `Let's explore ${topic} from the ground up.`,
          keyPoints: ["Core concept", "Why it matters", "Common use cases"],
          completed: false,
        },
        {
          stepNumber: 2,
          title: "Deeper Dive",
          content: `Now that we understand the basics, let's look at how ${topic} applies in practice.`,
          keyPoints: [
            "Real-world application",
            "Best practices",
            "Pitfalls to avoid",
          ],
          checkQuestion: `What is the primary benefit of understanding ${topic}?`,
          checkAnswer: "Better decision-making and fewer costly mistakes.",
          completed: false,
        },
        {
          stepNumber: 3,
          title: "Putting It Together",
          content: `Let's synthesize what we've learned about ${topic} and how to apply it.`,
          keyPoints: ["Integration", "Next steps", "Resources"],
          completed: false,
        },
      ],
    },
...
export function evaluateLearningAnswer(
  ...
  if (!step.checkAnswer)
    return { correct: true, feedback: "No check question for this step." };
  ...
  return {
    ...
    feedback:
      score >= 0.8
        ? "Excellent! You nailed the key concepts."
        : score >= 0.5
          ? "Good understanding. Consider: " + step.checkAnswer
          : "Not quite. Think about: " + step.checkAnswer,
  };
}
...
export function constrainResearch(
    ...
    if (constraint.type === "include") {
      if (constraint.domain)
        filtered = filtered.filter(
          (s) =>
            s.domain === constraint.domain ||
            s.url?.includes(constraint.domain!),
        );
      if (constraint.module)
        filtered = filtered.filter((s) => s.type === "document");
      if (constraint.confidenceThreshold)
        filtered = filtered.filter(
          (s) => s.relevanceScore >= constraint.confidenceThreshold!,
        );
    } else {
      if (constraint.domain)
        filtered = filtered.filter(
      ...
      if (constraint.documentId)
        filtered = filtered.filter((s) => s.id !== constraint.documentId);
    }
  ...
  if (filtered.length === 0)
    gaps.push(
      "No sources match the given constraints — try broadening your scope",
    );
  if (!filtered.some((s) => s.type === "document"))
    gaps.push(
      "No internal documents found — consider connecting relevant modules",
    );
  if (!filtered.some((s) => s.type === "web"))
    gaps.push("No web results — enable web search for broader coverage");
  ...
  return {
    ...
    synthesis:
      filtered.length > 0
        ? `Found ${filtered.length} relevant sources. Top source: "${filtered[0]!.title}" (${(filtered[0]!.relevanceScore * 100).toFixed(0)}% match)`
...
export function recordOutcome(
  record: Omit<OutcomeRecord, "id" | "timestamp">,
): OutcomeRecord {
  return {
    ...record,
...
export function summarizeOutcomes(records: OutcomeRecord[]): OutcomeSummary {
  ...
  const featureMap = new Map<
    string,
    { count: number; totalSatisfaction: number }
  >();
  ...
  const recentAvg =
    recent.reduce((a, r) => a + r.userSatisfaction, 0) / recent.length;
  const earlierAvg =
    earlier.length > 0
      ? earlier.reduce((a, r) => a + r.userSatisfaction, 0) / earlier.length
      : recentAvg;
  return {
    totalActions: records.length,
    avgTimeSavedMs:
      records.reduce((a, r) => a + r.timeSavedMs, 0) / records.length,
    avgDecisionQuality:
      records.reduce((a, r) => a + r.decisionQuality, 0) / records.length,
    avgFollowThrough:
      records.reduce((a, r) => a + r.followThroughRate, 0) / records.length,
    avgSatisfaction:
      records.reduce((a, r) => a + r.userSatisfaction, 0) / records.length,
    topFeatures: Array.from(featureMap.entries())
      .map(([feature, { count, totalSatisfaction }]) => ({
        feature,
        count,
...
export function recallMemories(
  memories: PersistentMemoryEntry[],
    ...
    results = results.filter((m) =>
      query.keywords!.some(
        (kw) =>
          ...
          m.tags.some((t) => t.toLowerCase().includes(kw.toLowerCase())),
      ),
    );
  }
...
export function buildContextGraph(
  ...
  for (const conv of conversations) {
    for (const msg of conv.messages) {
      ...
      const decisions = msg.content.match(
        /(?:decided|chosen|recommended|suggest)\s+([^.!?]+)/gi,
      );
      if (decisions) {
        for (const d of decisions) {
          const node: ContextNode3D = {
            id: `dec_${nodes.length}`,
            label: d.slice(0, 40),
            type: "decision",
            weight: 2,
            color: "#10b981",
  ...
  for (let i = 0; i < Math.min(decisionNodes.length, 5); i++) {
    const docNode =
      nodes[Math.floor(Math.random() * Math.min(documents.length, 15))];
    if (docNode && decisionNodes[i]) {
      edges.push({
        source: docNode.id,
        target: decisionNodes[i]!.id,
        relation: "informed",
        weight: 1,
      });
    }
  ...
  for (const mod of modules.slice(0, 4)) {
    const modNodes = nodes
      .filter((n) => n.metadata.module === mod)
      .map((n) => n.id);
    if (modNodes.length > 0)
      clusters.push({ id: `cluster_${mod}`, label: mod, nodeIds: modNodes });
  }
