export type ResolutionLevel =
  "concise" | "detailed" | "autonomous_plan" | "spatial_cue";
...
export class MultiResolutionRenderer {
  render(
    answer: string,
    level: ResolutionLevel,
    context: { confidence: number; sources: string[] },
  ): RenderedResponse {
    if (level === "concise") {
      return {
        level,
        content: answer.split(".")[0] + ".",
        metadata: { source: "synthesis", confidence: context.confidence },
      };
    }
    if (level === "detailed") {
      return {
    ...
    if (level === "autonomous_plan") {
      return {
        ...
        metadata: { source: "planner", confidence: context.confidence },
      };
    }
    return {
      level,
      content: "[Spatial] " + answer,
      metadata: { source: "xr_engine", confidence: context.confidence },
    };
  }
}
