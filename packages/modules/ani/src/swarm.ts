...
async function executeResearch(task: AgentTask, context: WorkspaceContext): Promise<string> {
...
async function executeGovernance(task: AgentTask, context: WorkspaceContext): Promise<string> {
...
async function executeSynthesis(task: AgentTask, priorResults: Array<{ role: AgentRole; output: string; confidence: number }>): Promise<string> {
...
export class MultiAgentSwarmOrchestrator {
  async decomposeGoal(goal: string, context: WorkspaceContext): Promise<SwarmPlan> {
    ...
    if (intent.includes("research") || intent.includes("analyze") || intent.includes("find")) {
      steps.push(makeTask("research", "Search workspace and external sources for relevant information"));
    }
    if (intent.includes("code") || intent.includes("implement") || intent.includes("create") || intent.includes("build")) {
      steps.push(makeTask("code", "Generate or modify code based on requirements"));
    }
    if (intent.includes("workflow") || intent.includes("automate") || intent.includes("orchestrate")) {
      steps.push(makeTask("execution", "Execute multi-step workflow across integrations"));
    }
    steps.push(makeTask("governance", "Verify compliance, check PII, validate safety"));
    if (steps.length === 0) {
      steps.push(makeTask("synthesis", "Synthesize information and generate response"));
    }
    steps.push(makeTask("synthesis", "Compile final response from all agent outputs"));

    return { id: planId, goal, steps, requiredConsensus: CONSENSUS_THRESHOLD, status: "planning" };
  }

  async executePlan(plan: SwarmPlan, context: WorkspaceContext): Promise<SwarmResult> {
