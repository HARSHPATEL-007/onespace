...
export class ContextFusionLayer {
  fuse(sources: Array<{ source: string; module: string; content: string; timestamp?: string }>): FusedWorkspaceModel {
    const items: FusedContextItem[] = sources.map((s) => {
      const recency = this._calcRecency(s.timestamp ?? new Date().toISOString());
      ...
      return { ...s, relevanceScore: relevance, recencyScore: recency, authorityScore: authority, fusedScore: fused, timestamp: s.timestamp ?? new Date().toISOString() };
    });
    ...
    return {
      ...
      contextSummary: items.slice(0, 3).map((i) => i.content.slice(0, 80)).join(" | "),
  ...
  private _calcAuthority(module: string): number {
    const authorityMap: Record<string, number> = { docs: 0.95, mail: 0.9, calendar: 0.85, tasks: 0.8, sheets: 0.75, crm: 0.88, meeting: 0.85 };
  ...
  private _extractTopEntities(items: FusedContextItem[]): string[] {
    ...
    for (const item of items) {
      const words = item.content.split(/\s+/).filter((w) => w.length > 4).slice(0, 3);
