export type ConversationPhase =
  "intake" | "clarify" | "plan" | "act" | "verify" | "handoff";
...
export class ConversationStateMachine {
  ...
  private transitions: PhaseTransition[] = [
    {
      from: "intake",
      to: "clarify",
      condition: "ambiguous or missing parameters",
    },
    {
      from: "intake",
      to: "plan",
      condition: "complex multi-step task identified",
    },
    ...
    { from: "clarify", to: "plan", condition: "user provided clarification" },
    {
      from: "clarify",
      to: "intake",
      condition: "new information changes scope",
    },
    ...
    { from: "act", to: "verify", condition: "action completed" },
    {
      from: "act",
      to: "plan",
      condition: "unexpected result requires replanning",
    },
  ...
  canTransitionTo(targetPhase: ConversationPhase): boolean {
    return this.transitions.some(
      (t) => t.from === this.currentPhase && t.to === targetPhase,
    );
  }
  ...
  transition(targetPhase: ConversationPhase, note = ""): boolean {
    ...
    this.history.push({
      ...
      note: from + " -> " + targetPhase + (note ? ": " + note : ""),
    });
    return true;
  }
