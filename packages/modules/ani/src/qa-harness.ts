...
export class ContinuousQAHarness {
  ...
  score(response: string, sources: string[], userFeedback?: number): QAScore {
    ...
    const safety =
      response.includes("password") || response.includes("secret") ? 0.3 : 0.95;
  ...
  shouldRetrain(): boolean {
    ...
    const avgGroundedness =
      recent.reduce((s, r) => s + r.groundedness, 0) / recent.length;
    const avgSafety = recent.reduce((s, r) => s + r.safety, 0) / recent.length;
    return (
      avgGroundedness < this.thresholds.groundedness ||
      avgSafety < this.thresholds.safety
    );
  }
}
