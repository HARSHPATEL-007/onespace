...
export interface TurnResponse {
  ...
  response: {
    body: string; checkForUnderstanding: string | null; misconceptionCheck: string | null;
...
export interface TutorAgentActions {
  turn: (input: { sessionId?: string; setId: string; conceptId?: string; message: string }) => Promise<TurnResponse>;
...
export function TutorAgentsPanel({ setId, concepts, actions, isInstructor }: {
  ...
  const send = () => {
    ...
    setMessage("");
    setBusy(true);
    void actions.turn({ sessionId, setId, conceptId: conceptId || undefined, message: q })
      .then((r) => {
      ...
      {turns.slice().reverse().map((t, i) => (
        <div key={`${t.r.sessionId}-${i}`} className="nv-card" style={{ fontSize: 13 }}>
          ...
          <div style={{ fontSize: 11, color: "var(--nv-color-text-faint)", marginTop: 2 }}>
            intent {t.r.intent} · {t.r.workflow} · {t.r.latencyMs}ms{t.r.response.metadata.humanReviewRequired ? " · 👩‍🏫 human review flagged" : ""}
          </div>
