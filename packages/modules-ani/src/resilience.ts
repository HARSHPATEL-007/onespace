...
export const DEFAULT_RETRY_CONFIG: RetryConfig = {
  ...
  maxDelayMs: 10000,
  backoffMultiplier: 2,
  retryableErrors: ["ECONNRESET", "ETIMEDOUT", "rate_limit", "timeout", "network", "503", "429"],
};
...
export async function withRetry<T>(
  ...
  return { result: null, attempts: cfg.maxAttempts, totalDurationMs: Date.now() - startTime, errors, degraded: true };
}
...
export class GracefulDegradation {
  ...
  registerFeature(name: string, enabled: boolean, healthCheck?: () => Promise<boolean>): void {
  ...
  getDegradedResponse(featureName: string): { available: boolean; fallback: string; message: string } {
    const fallbacks: Record<string, { fallback: string; message: string }> = {
      deep_think: { fallback: "standard", message: "Deep Think temporarily unavailable — using standard mode" },
      voice_input: { fallback: "text", message: "Voice input unavailable — please type your message" },
      voice_output: { fallback: "text", message: "Voice output unavailable — response shown as text" },
      graph_3d: { fallback: "list", message: "3D visualization unavailable — showing list view" },
      meeting_intel: { fallback: "basic", message: "Meeting intelligence unavailable — basic mode active" },
      real_time_stream: { fallback: "batch", message: "Streaming unavailable — loading complete response" },
    };
    const fb = fallbacks[featureName] ?? { fallback: "basic", message: "Feature unavailable — using basic mode" };
    return { available: false, ...fb };
  }
}

export class ProductionMonitor {
  private metrics: Array<{ timestamp: string; name: string; value: number; tags: Record<string, string> }> = [];

  record(name: string, value: number, tags: Record<string, string> = {}): void {
    this.metrics.push({ timestamp: new Date().toISOString(), name, value, tags });
    if (this.metrics.length > 10000) this.metrics = this.metrics.slice(-5000);
  }

  getStats(name: string, windowMs: number = 300000): { avg: number; min: number; max: number; count: number; p95: number } {
    const cutoff = Date.now() - windowMs;
    const values = this.metrics
      .filter((m) => m.name === name && new Date(m.timestamp).getTime() > cutoff)
      .map((m) => m.value)
      .sort((a, b) => a - b);
  ...
  getHealth(): { status: "healthy" | "degraded" | "critical"; issues: string[] } {
    ...
    if (latencyStats.p95 > 10000) issues.push(`High p95 latency: ${latencyStats.p95}ms`);
    if (errorRate.avg > 0.1) issues.push(`High error rate: ${(errorRate.avg * 100).toFixed(1)}%`);

    return {
      status: issues.length === 0 ? "healthy" : issues.length > 1 ? "critical" : "degraded",
      issues,
    };
  }
}
