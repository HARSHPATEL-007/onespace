...
export class CausalReasoningEngine {
  ...
  addCausalLink(source: string, target: string, strength: number, mechanism: string): void {
    ...
    if (!this.nodes.find((n) => n.name === source)) this.nodes.push({ id: `node_${source}`, name: source, value: 0, confidence: 0.9 });
    if (!this.nodes.find((n) => n.name === target)) this.nodes.push({ id: `node_${target}`, name: target, value: 0, confidence: 0.9 });
  }
