  ...
  try {
    ...
    return NextResponse.json({
      ...
      results: results.map((r) => ({
        ...
        source_date: r.citation.sourceDate?.toISOString() ?? null,
        retrieved_at: r.citation.freshnessAt?.toISOString() ?? null,
        authority_score: r.citation.authority / 100,
        freshness_score: r.citation.freshnessScore ?? null,
        extraction_confidence: r.citation.extractionConfidence,
        evidence_type: r.citation.evidenceType,
        language: r.citation.language || null,
        license: r.citation.license || null,
