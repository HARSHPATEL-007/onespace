...
export async function POST(request: NextRequest) {
  ...
  const body = await request.json().catch(() => ({})); const { asset_id, levels, thresholds } = body;
  ...
  const result = await svc.srDuplicateSearch({ asset_id: String(asset_id), levels: levels?.map(String), thresholds, tenant_id: membership.workspaceId });
