...
export async function POST(request: NextRequest, { params }: { params: Promise<{ findingId: string }> }) {
  ...
  const proposals = await svc.qualityProposalsForFinding(findingId);
  return NextResponse.json({ findingId, proposals });
}
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ findingId: string }> }) {
  ...
  if (!membership) return NextResponse.json({ error: "No workspace" }, { status: 403 });
  const svc = new VideosService(membership.workspaceId, session.user.id, membership.role);
  const finding = await svc.qualityGetFinding(findingId);
  if (!finding) return NextResponse.json({ error: "Finding not found" }, { status: 404 });
  const proposals = await svc.qualityProposalsForFinding(findingId);
  return NextResponse.json({ finding, proposals });
}
